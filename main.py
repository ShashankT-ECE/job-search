...
Stages:
  ...
  2. Gemini rank → scored & filtered jobs        (future)
...
def main():
    ...
    parser.add_argument(
        "--rank",
        action="store_true",
        help="(Future) Run Gemini ranking on scraped jobs",
    )
    parser.add_argument(
        "--tailor",
        action="store_true",
    ...
    if not any([args.scrape, args.build_resume, args.rank, args.tailor, args.dashboard]):
        parser.print_help()
        return
    ...
    # ── Future stubs ─────────────────────────────────────────────────
    if args.rank:
        print("Ranking via Gemini — not yet implemented.")
        sys.exit(1)
